...
@section('content')
    <div class="space-y-4">
        ...
        <nav class="flex flex-wrap items-center gap-x-2 gap-y-1 text-sm text-body dark:text-slate-400 pt-4" aria-label="{{ __('messages.nav.breadcrumb') }}">
            ...
            <a href="{{ route('drugs.medical-articles.index') }}" wire:navigate class="hover:text-fg-brand dark:hover:text-brand transition-colors">{{ __('drugs.breadcrumb.articles') }}</a>
            <x-lucide-chevron-right class="w-4 h-4 rtl:rotate-180 shrink-0" />
        ...
        <div class="grid grid-cols-1 lg:grid-cols-12 gap-4">
                    ...
                    <div class="px-4 sm:px-8 py-8">
                        <div class="max-w-2xl">
                            ...
                            <div class="mt-10 pt-6 border-t border-default-medium dark:border-slate-700">
                                <a href="{{ route('drugs.medical-articles.index') }}" wire:navigate
                                    ...
                                    {{ __('drugs.medical_articles.back') }}
                                </a>
                            </div>
                        </div>
                    </div>
            ...
            <div class="lg:col-span-3">
                <div class="bg-neutral-primary-soft rounded-base shadow-xs p-5 dark:bg-slate-800 lg:sticky lg:top-20">
                    <h2 class="text-sm font-semibold uppercase tracking-wider text-body dark:text-slate-400 mb-3">{{ __('drugs.medical_articles.toc') }}</h2>
                    ...
                    @else
                        <p class="text-sm text-body dark:text-slate-400">{{ __('drugs.medical_articles.no_toc') }}</p>
                    @endif
                </div>
            </div>
