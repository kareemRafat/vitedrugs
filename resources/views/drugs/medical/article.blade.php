...
@section('title', __('drugs.medical.title', ['name' => $article['title']]))
...
@section('content')
    <div class="space-y-4">
        ...
        <nav class="flex flex-wrap items-center gap-x-2 gap-y-1 text-sm text-body dark:text-slate-400 pt-4" aria-label="{{ __('messages.nav.breadcrumb') }}">
            ...
            <a href="{{ route('drugs.diseases.show', $disease->slug) }}" wire:navigate class="hover:text-fg-brand dark:hover:text-brand transition-colors">{{ __('drugs.breadcrumb.diseases') }}</a>
            <x-lucide-chevron-right class="w-4 h-4 rtl:rotate-180 shrink-0" />
        ...
        <div class="grid grid-cols-1 lg:grid-cols-12 gap-4">
                ...
                <article class="bg-neutral-primary-soft rounded-base shadow-xs overflow-hidden dark:bg-slate-800 dark:border dark:border-slate-700">
                    <div class="px-4 sm:px-8 py-8">
                        <div class="max-w-3xl">
                                                        ...
                                                        @if (! empty($item['meta']))
                                                            <div class="mt-2 flex flex-wrap gap-1.5">
                                                                @if (! empty($item['meta']['is_pathognomonic']))
                                                                    <span class="px-2 py-0.5 text-xs font-semibold rounded-xs bg-brand text-white">{{ __('drugs.disease.pathognomonic') }}</span>
                                                                @endif
                                                                @if (! empty($item['meta']['is_required']))
                                                                    <span class="px-2 py-0.5 text-xs font-semibold rounded-xs bg-brand-soft text-fg-brand dark:bg-brand/20 dark:text-brand">{{ __('drugs.disease.required') }}</span>
                                                                @endif
                                                                @if (! empty($item['meta']['is_specific']))
                                                                    <span class="px-2 py-0.5 text-xs font-semibold rounded-xs bg-brand-soft text-fg-brand dark:bg-brand/20 dark:text-brand">{{ __('drugs.disease.specific') }}</span>
                                                                @endif
                                                                @if (! empty($item['meta']['weight']))
                                                                    <span class="px-2 py-0.5 text-xs font-semibold rounded-xs bg-neutral-secondary-soft border border-default-medium text-body dark:bg-slate-700 dark:border-slate-600 dark:text-slate-300">{{ __('drugs.disease.weight') }}: {{ $item['meta']['weight'] }}</span>
                                                                @endif
                                                            </div>
                                                        @endif
                                                    </div>
                            ...
                            @empty
                                <p class="text-base text-body dark:text-slate-400">{{ __('drugs.medical.no_sections') }}</p>
                            @endforelse
            ...
            <div class="lg:col-span-3">
                <div class="bg-neutral-primary-soft rounded-base shadow-xs p-5 dark:bg-slate-800 lg:sticky lg:top-20">
                    <h2 class="text-sm font-semibold uppercase tracking-wider text-body dark:text-slate-400 mb-3">{{ __('drugs.medical.on_this_page') }}</h2>
                    ...
                    @else
                        <p class="text-sm text-body dark:text-slate-400">{{ __('drugs.medical.no_sections') }}</p>
                    @endif
                </div>
            </div>
