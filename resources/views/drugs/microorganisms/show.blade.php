...
@section('content')
    ...
    <div class="space-y-4">
        ...
        <nav class="flex flex-wrap items-center gap-x-2 gap-y-1 text-sm text-body dark:text-slate-400 pt-4" aria-label="{{ __('messages.nav.breadcrumb') }}">
            ...
            <a href="{{ route('drugs.microorganisms.index') }}" wire:navigate class="hover:text-fg-brand dark:hover:text-brand transition-colors">{{ __('drugs.breadcrumb.microorganisms') }}</a>
            <x-lucide-chevron-right class="w-4 h-4 rtl:rotate-180 shrink-0" />
            ...
            <div class="lg:col-span-8 space-y-4">
                ...
                @if ($causes->isNotEmpty())
                    <div class="bg-neutral-primary-soft rounded-base shadow-xs dark:bg-slate-800 overflow-hidden">
                        <div class="px-5 py-4 border-b border-default-medium flex items-center gap-2">
                            ...
                            <h2 class="text-base font-semibold text-heading dark:text-white">{{ __('drugs.microorganisms.causes') }}</h2>
                ...
                @if ($associated->isNotEmpty())
                    <div class="bg-neutral-primary-soft rounded-base shadow-xs dark:bg-slate-800 overflow-hidden">
                        <div class="px-5 py-4 border-b border-default-medium flex items-center gap-2">
                            ...
                            <h2 class="text-base font-semibold text-heading dark:text-white">{{ __('drugs.microorganisms.associated') }}</h2>
                ...
                @if ($microorganism->diseases->isEmpty())
                    <div class="bg-neutral-primary-soft rounded-base shadow-xs p-10 text-center dark:bg-slate-800">
                        ...
                        <p class="text-base text-body dark:text-slate-400">{{ __('drugs.microorganisms.no_diseases') }}</p>
                    </div>
                @endif
            ...
            <div class="lg:col-span-4 space-y-4">
                <div class="bg-neutral-primary-soft rounded-base shadow-xs dark:bg-slate-800 overflow-hidden">
                    <div class="px-5 py-4 border-b border-default-medium flex items-center gap-2">
                        ...
                        <h2 class="text-base font-semibold text-heading dark:text-white">{{ __('drugs.microorganisms.taxonomy') }}</h2>
                    </div>
