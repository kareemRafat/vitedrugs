...
@section('content')
    ...
    <div class="space-y-4">
        <x-drugs.page-hero :heading="__('drugs.diagnosis.results_heading')" :subtitle="__('drugs.diagnosis.results_subtitle')" :badge="__('drugs.hero.badge.results')" badgeIcon="stethoscope">
            <a href="{{ route('drugs.diagnosis') }}" class="inline-flex items-center gap-2 px-4 py-2 text-sm font-medium bg-white/15 text-white border border-white/30 rounded-base hover:bg-white/25"><x-lucide-rotate-ccw class="w-4 h-4" />{{ __('drugs.diagnosis.restart') }}</a>
        </x-drugs.page-hero>
