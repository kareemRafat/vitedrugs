...
@section('title', __('large-animals.projects.title'))
...
@section('content')
    <div class="space-y-4">
        ...
        <x-large-animals.page-hero
            :heading="__('large-animals.projects.heading')"
            :subtitle="__('large-animals.projects.subtitle')"
            :badge="__('large-animals.hero.badge.projects')"
            badgeIcon="folder-kanban"
            :stats="[
                ['count' => $projects->total(), 'label' => __('large-animals.hero.stats.projects'), 'icon' => 'folder-kanban'],
            ]"
        />
        ...
        @if ($projects->isEmpty())
            ...
            <div class="bg-neutral-primary-soft rounded-base shadow-xs p-12 text-center dark:bg-slate-800">
                ...
                <h2 class="text-xl font-semibold text-heading dark:text-white mb-2">{{ __('large-animals.projects.no_projects') }}</h2>
                <p class="text-base text-body dark:text-slate-400">{{ __('large-animals.projects.no_projects_desc') }}</p>
            </div>
        ...
        @else
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                @foreach ($projects as $project)
                    <article class="group bg-neutral-primary-soft rounded-base shadow-xs overflow-hidden border border-default-medium hover:shadow-md hover:border-brand transition-all duration-300 dark:bg-slate-800 dark:border-slate-700 flex flex-col">
                        <a href="{{ route('large-animals.projects.show', $project->slug) }}" wire:navigate class="flex flex-col h-full">
                            ...
                            <div class="p-5 flex flex-col flex-1">
                                ...
                                <div class="flex items-center gap-1.5 text-sm font-medium text-fg-brand pt-4 mt-auto">
                                    {{ __('large-animals.projects.read_more') }}
            ...
            <x-pagination :paginator="$projects" translation-prefix="large-animals.common" />
