        ...
        <div class="grid grid-cols-1 lg:grid-cols-12 gap-6">
            <div class="lg:col-span-7 xl:col-span-8">
                ...
                @if ($company->name_ar && app()->getLocale() === 'ar')
                    <p class="text-body dark:text-gray-400 text-base mb-2">{{ $company->name_ar }}</p>
                @endif
                <div class="flex flex-wrap gap-2 mb-3">
                    ...
                    @if ($company->country)
                        <span class="inline-flex items-center px-2.5 py-0.5 rounded-base text-sm font-medium bg-neutral-secondary-soft text-heading border border-default-medium dark:bg-gray-700 dark:text-white dark:border-gray-600">
                ...
                @if ($company->description)
                    <p class="text-base text-body dark:text-gray-400">{{ $company->description }}</p>
                @endif
            </div>
            ...
            <div class="lg:col-span-5 xl:col-span-4">
                <div class="grid grid-cols-2 gap-3">
                    <div class="bg-neutral-secondary-soft rounded-base p-3 border border-default-medium shadow-sm dark:bg-gray-700 dark:border-gray-600">
                        <span class="block text-base uppercase text-body dark:text-gray-400 mb-1">{{ __('messages.companies.type') }}</span>
                    ...
                    <div class="bg-neutral-secondary-soft rounded-base p-3 border border-default-medium shadow-sm dark:bg-gray-700 dark:border-gray-600">
                        <span class="block text-base uppercase text-body dark:text-gray-400 mb-1">{{ __('messages.companies.country') }}</span>
                        <span class="font-semibold text-heading dark:text-white text-base">{{ $company->country ?? __('messages.companies.na') }}</span>
                    </div>
                    <div class="bg-neutral-secondary-soft rounded-base p-3 border border-default-medium shadow-sm dark:bg-gray-700 dark:border-gray-600">
                        <span class="block text-base uppercase text-body dark:text-gray-400 mb-1">{{ __('messages.companies.products_count') }}</span>
                        <span class="font-semibold text-heading dark:text-white text-base">{{ $company->products->count() }}</span>
                    </div>
                    <div class="bg-neutral-secondary-soft rounded-base p-3 border border-default-medium shadow-sm dark:bg-gray-700 dark:border-gray-600">
                        <span class="block text-base uppercase text-body dark:text-gray-400 mb-1">{{ __('messages.companies.registration') }}</span>
            ...
            <div class="bg-neutral-primary-soft rounded-base shadow-sm dark:bg-gray-800 overflow-hidden">
                ...
                <div class="overflow-x-auto">
                    @if ($company->products->isEmpty())
                        <p class="text-base text-body dark:text-gray-400 text-center py-8">{{ __('messages.companies.no_products') }}</p>
                    @else
                ...
                <div class="p-5 space-y-3">
                    ...
                    @foreach ($contactFields as $field => $meta)
                        @if ($company->$field)
                            <div class="flex items-start gap-3">
                                ...
                                <div class="min-w-0">
                                    <span class="block text-sm uppercase text-body dark:text-gray-400">{{ $meta['label'] }}</span>
                    ...
                    @if ($company->address)
                        <div class="flex items-start gap-3 pt-3 border-t border-default-medium">
                            ...
                            <div class="min-w-0">
                                <span class="block text-sm uppercase text-body dark:text-gray-400">{{ __('messages.companies.address') }}</span>
