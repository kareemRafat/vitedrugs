...
<div class="space-y-6">
    <x-drugs.page-hero
        ...
        badgeIcon="columns-3"
        :stats="[
            ['count' => $this->totalCount, 'label' => __('drugs.comparison.selected'), 'icon' => 'columns-3'],
        ]"
    />
                ...
                <div class="p-4 border-b border-default-medium dark:border-slate-700">
                    <div class="relative">
                        ...
                        <input type="text" wire:model.live.debounce.300ms="{{ $searchProp }}"
                            placeholder="{{ __('drugs.comparison.search_placeholder') }}"
                        ...
                        <div class="relative flex gap-3">
                            ...
                            <div class="flex-1 min-w-0 pe-6">
                                ...
                                <div class="text-xs text-body dark:text-slate-400 mt-1 space-y-0.5">
                                    <p>{{ $disease->hostSpecies->count() }} {{ __('drugs.hero.stats.species') }}</p>
                                    <p>{{ $disease->clinicalSigns->count() }} {{ __('drugs.hero.stats.clinical_signs') }}</p>
                                </div>
                            </div>
                        </div>
                    @else
                        <div class="flex-1 flex items-center justify-center">
                            <div class="text-center">
                                ...
                                <p class="text-sm text-body/60 dark:text-slate-500">
                                    {{ __('drugs.comparison.search_to_add') }}
                                </p>
                            </div>
                        </div>
                    @endif
    ...
    @if ($this->totalCount > 0)
        <div class="flex justify-end">
            <button type="button" wire:click="clearAll" wire:loading.attr="disabled"
                ...
                <span wire:loading.remove wire:target="clearAll">{{ __('drugs.comparison.clear_all') }}</span>
                <span wire:loading wire:target="clearAll">{{ __('drugs.comparison.clearing') }}</span>
            </button>
        </div>
    @endif
    ...
    @php
        $comparisonTabs = collect([
            ['id' => 'overview', 'label' => __('drugs.comparison.overview'), 'icon' => 'clipboard-list', 'type' => 'overview', 'rows' => $this->overviewMatrix],
            ['id' => 'clinical_signs', 'label' => __('drugs.comparison.clinical_signs'), 'icon' => 'stethoscope', 'type' => 'presence', 'rows' => $this->clinicalSignsMatrix],
            ['id' => 'postmortem_findings', 'label' => __('drugs.comparison.postmortem_findings'), 'icon' => 'microscope', 'type' => 'presence', 'rows' => $this->postmortemMatrix],
            ['id' => 'diagnosis', 'label' => __('drugs.comparison.diagnosis'), 'icon' => 'test-tube-2', 'type' => 'presence', 'rows' => $this->diagnosisMatrix],
            ['id' => 'treatment', 'label' => __('drugs.comparison.treatment'), 'icon' => 'syringe', 'type' => 'presence', 'rows' => $this->treatmentMatrix],
            ['id' => 'prevention', 'label' => __('drugs.comparison.prevention'), 'icon' => 'shield-check', 'type' => 'presence', 'rows' => $this->preventionMatrix],
            ['id' => 'references', 'label' => __('drugs.comparison.references'), 'icon' => 'book-marked', 'type' => 'presence', 'rows' => $this->referencesMatrix],
        ])
            ...
            <div class="bg-neutral-primary-soft dark:bg-slate-800 rounded-base shadow-xs p-3">
                <div class="flex flex-wrap items-center justify-between gap-3">
                    ...
                    <label class="inline-flex items-center gap-2 cursor-pointer select-none">
                        ...
                        <span class="text-sm font-medium text-heading dark:text-white">{{ __('drugs.comparison.show_differences_only') }}</span>
                    </label>
                </div>
            </div>
                ...
                <div x-show="active === '{{ $tab['id'] }}'" x-cloak class="bg-neutral-primary-soft dark:bg-slate-800 rounded-base shadow-xs overflow-hidden">
                    <div class="px-5 py-4 border-b border-default-medium flex items-center justify-between gap-3">
                        ...
                        <span class="text-xs text-body dark:text-slate-400">
                            <span x-show="!diffOnly">{{ $tab['rowCount'] }} {{ __('drugs.comparison.items') }}</span>
                            <span x-show="diffOnly" x-cloak>{{ $tab['diffCount'] }} {{ __('drugs.comparison.different_items') }}</span>
                        </span>
                    </div>
                    ...
                    <div class="overflow-x-auto">
                            ...
                            <thead class="bg-neutral-secondary-soft dark:bg-slate-700">
                                <tr>
                                    <th class="px-5 py-4 text-xs uppercase tracking-wider text-body dark:text-slate-400 font-semibold w-48">
                                        {{ __('drugs.comparison.attribute') }}
                                    </th>
                                    @foreach ($this->selectedDiseases as $disease)
                                        <th class="px-5 py-4 min-w-[200px] text-sm font-bold text-heading dark:text-white">
                                            <a href="{{ route('drugs.diseases.show', $disease->slug) }}" wire:navigate class="hover:text-brand dark:hover:text-sky-400 transition-colors">{{ $disease->name }}</a>
                                        </th>
                                    @endforeach
                                </tr>
                            </thead>
                                    ...
                                    <tr x-show="!diffOnly || @js($item['differs'])">
                                        @if ($tab['type'] === 'overview')
                                            <td class="px-5 py-4 text-sm font-medium text-heading dark:text-white bg-neutral-secondary-soft/50 dark:bg-slate-700/50">
                                                {{ __('drugs.comparison.' . $item['name']) }}
                                            </td>
                                            @foreach ($this->selectedDiseases as $disease)
                                                <td class="px-5 py-4">
                                                    @if ($item['type'] === 'boolean')
                                                        <span class="text-sm font-medium text-heading dark:text-white">
                                                            {{ ($item['values'][$disease->id] ?? false) ? __('drugs.disease.yes') : __('drugs.disease.no') }}
                                                        </span>
                                                    @else
                        ...
                        <p x-show="diffOnly && @js($tab['diffCount'] === 0)" x-cloak class="text-sm text-body dark:text-slate-400 py-8 text-center">
                            {{ __('drugs.comparison.no_differences') }}
                        </p>
                    </div>
                </div>
            @endforeach
    ...
    @elseif ($this->totalCount === 1)
        <div class="bg-neutral-primary-soft dark:bg-slate-800 rounded-base shadow-xs px-6 py-10 text-center">
            ...
            <p class="text-sm text-body dark:text-slate-400">{{ __('drugs.comparison.select_more') }}</p>
        </div>
    ...
    @else
        <div class="bg-neutral-primary-soft dark:bg-slate-800 rounded-base shadow-xs py-16 text-center">
            ...
            <h3 class="text-xl font-semibold text-heading dark:text-white mb-2">
                {{ __('drugs.comparison.empty_title') }}
            </h3>
            <p class="text-sm text-body dark:text-slate-400 mb-6">
                {{ __('drugs.comparison.empty_desc') }}
            </p>
            <button type="button" wire:click="browseDiseases"
                ...
                {{ __('drugs.comparison.browse_diseases') }}
            </button>
        </div>
    @endif
</div>
