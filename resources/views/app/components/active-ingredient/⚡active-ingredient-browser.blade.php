...
<div class="space-y-4">
    ...
    <x-page-hero
        ...
        :stats="[
            ['count' => number_format($this->totalCount), 'label' => __('messages.active_ingredients.ingredients_label'), 'icon' => 'flask-conical'],
        ]"
    />
        ...
        <div class="grid grid-cols-1 sm:grid-cols-3 gap-3">
            ...
            <div class="flex justify-between gap-2">
                <button type="button" wire:click="clear"
                    ...
                    {{ __('messages.active_ingredients.clear_filter') }}
                </button>
            </div>
        </div>
    </div>
    ...
    <div class="flex items-center justify-between">
        <div class="text-sm text-body dark:text-gray-400">
            @if ($activeLetter)
                {{ $this->ingredients->count() }} {{ __('messages.active_ingredients.of') }} {{ $this->totalCount }} {{ __('messages.active_ingredients.ingredients_label') }}
                {{ __('messages.active_ingredients.starting_with', ['letter' => $activeLetter]) }}
            @elseif ($this->search)
                {{ $this->ingredients->count() }} {{ __('messages.active_ingredients.of') }} {{ $this->totalCount }} {{ __('messages.active_ingredients.ingredients_label') }}
            @else
                {{ $this->ingredients->count() }} {{ __('messages.active_ingredients.of') }} {{ $this->totalCount }} {{ __('messages.active_ingredients.ingredients_label') }}
            ...
            @foreach ($this->ingredients as $ingredient)
                <div class="group bg-white dark:bg-gray-800 rounded-xl border border-neutral-200 dark:border-gray-700 shadow-sm hover:shadow-lg transition-all duration-300 flex flex-col relative overflow-hidden">
                    ...
                    <div class="p-5 pt-6 flex flex-col flex-1">
                        <a href="{{ route('active-ingredients.show', $ingredient) }}" class="text-lg font-semibold text-heading dark:text-white group-hover:text-brand transition-colors leading-tight">
                            {{ $ingredient->name }}
                        </a>

                        @if ($ingredient->name_ar && app()->getLocale() === 'ar')
                            <p class="text-sm text-body dark:text-gray-400 mt-1">{{ $ingredient->name_ar }}</p>
                        @endif
                        ...
                        <div class="mt-auto pt-4 flex items-center gap-1.5 text-sm text-body dark:text-gray-400">
                            ...
                            <span>{{ $ingredient->products_count }} {{ __('messages.active_ingredients.products_count') }}</span>
                        </div>
                    </div>

                    <a href="{{ route('active-ingredients.show', $ingredient) }}" class="border-t border-default-medium dark:border-gray-700 px-5 py-3 flex items-center justify-between text-sm font-medium text-body hover:text-brand dark:text-gray-400 dark:hover:text-brand hover:bg-neutral-secondary-soft dark:hover:bg-gray-700/50 transition-colors">
                        <span class="flex items-center gap-2">
                            <x-lucide-eye class="w-4 h-4" />
                            {{ __('messages.active_ingredients.details') }}
                        </span>
            ...
            <div class="flex items-center justify-between bg-neutral-primary-soft rounded-base shadow-xs px-5 py-3 dark:bg-gray-800">
                ...
                <div class="flex items-center gap-2">
                    <button type="button" wire:click="goToPage({{ $this->currentPage - 1 }})" @if($this->currentPage === 1) disabled @endif
                        ...
                        <span class="ms-1">{{ __('messages.active_ingredients.previous_page') }}</span>
                    </button>

                    <button type="button" wire:click="goToPage({{ $this->currentPage + 1 }})" @if(!$this->hasMorePages) disabled @endif
                        class="inline-flex items-center px-3 py-2 text-sm font-medium text-heading bg-neutral-primary-soft border border-default-medium rounded-base hover:bg-neutral-secondary-soft disabled:opacity-40 disabled:cursor-not-allowed dark:bg-gray-700 dark:text-white dark:border-gray-600 dark:hover:bg-gray-600 transition-colors">
                        <span class="me-1">{{ __('messages.active_ingredients.next_page') }}</span>
    ...
    @else
        <div class="bg-neutral-primary-soft rounded-base shadow-xs dark:bg-gray-800 py-16 text-center">
            <x-lucide-flask-conical class="w-12 h-12 text-body mx-auto mb-4" />
            <h3 class="text-lg font-semibold text-heading dark:text-white mb-1">{{ __('messages.active_ingredients.no_ingredients') }}</h3>
            <p class="text-sm text-body dark:text-gray-400">{{ __('messages.active_ingredients.try_another') }}</p>
        </div>
    @endif
</div>
