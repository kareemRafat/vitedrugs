            ...
            <main class="flex-1 min-w-0">
                <div class="bg-neutral-primary-soft rounded-base shadow-xs border border-default-medium dark:bg-slate-800 dark:border-slate-700 p-6">
                    ...
                    <form method="POST" action="{{ route('profile.update') }}" class="space-y-5">
                        ...
                        <div>
                            <label for="phone" class="block mb-2 text-sm font-medium text-heading dark:text-white">{{ __('messages.profile.phone_label') }}</label>
                            <div class="relative">
                                <div class="absolute inset-y-0 start-0 flex items-center ps-3.5 pointer-events-none">
                                ...
                                <input type="tel" name="phone" id="phone" value="{{ old('phone', $user->phone) }}" required autocomplete="tel"
                                    class="bg-neutral-secondary-medium border border-default-medium text-heading text-sm rounded-base focus:ring-brand focus:border-brand block w-full ps-10 px-3 py-2.5 shadow-xs placeholder:text-body dark:bg-slate-700 dark:border-slate-600 dark:text-white @error('phone') border-red-600 @enderror"
                                    placeholder="{{ __('messages.profile.phone_placeholder') }}">
                            </div>
                            @error('phone')
                                <p class="mt-1 text-xs sm:text-sm font-medium text-fg-danger-strong flex items-center gap-1">
                                    <x-lucide-alert-circle class="w-3 h-3 shrink-0" />
