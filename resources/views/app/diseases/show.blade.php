        ...
        <div class="lg:col-span-7 xl:col-span-8 space-y-4">
            ...
            <div class="bg-neutral-primary-soft rounded-base shadow-sm p-4 sm:p-6 dark:bg-gray-800">
                ...
                @if ($disease->name_ar && app()->getLocale() === 'ar')
                    <p class="text-body dark:text-gray-400 text-base mb-2">{{ $disease->name_ar }}</p>
                @endif
                @if ($disease->description)
                    <p class="text-base text-body dark:text-gray-400">{{ $disease->description }}</p>
                @endif
                ...
                <div class="grid grid-cols-2 gap-3 mt-4">
                    <div class="bg-neutral-secondary-soft rounded-base p-3 border border-default-medium shadow-sm dark:bg-gray-700 dark:border-gray-600">
                        <span class="block text-base uppercase text-body dark:text-gray-400 mb-1">{{ __('messages.diseases.related_products') }}</span>
                        <span class="font-semibold text-heading dark:text-white text-base">{{ $disease->products->count() }}</span>
                    </div>
                    <div class="bg-neutral-secondary-soft rounded-base p-3 border border-default-medium shadow-sm dark:bg-gray-700 dark:border-gray-600">
                        <span class="block text-base uppercase text-body dark:text-gray-400 mb-1">{{ __('messages.diseases.ingredients_count') }}</span>
            ...
            <div class="bg-neutral-primary-soft rounded-base shadow-sm dark:bg-gray-800 overflow-hidden">
                ...
                <div class="overflow-x-auto">
                    @if ($disease->products->isEmpty())
                        <p class="text-base text-body dark:text-gray-400 text-center py-8">{{ __('messages.diseases.no_products') }}</p>
                    @else
                                ...
                                @foreach ($disease->products as $product)
                                    <tr class="border-b border-default-medium dark:border-gray-700">
                                        ...
                                        <td class="px-5 py-4 hidden md:table-cell">
                                            ...
                                            @else
                                                <span class="text-body dark:text-gray-400">{{ __('messages.diseases.na') }}</span>
                                            @endif
                                        </td>
                                        <td class="px-5 py-4">
            ...
            <div class="bg-neutral-primary-soft rounded-base shadow-sm dark:bg-gray-800">
                ...
                <div class="p-5">
                    @if ($ingredients->isEmpty())
                        <p class="text-base text-body dark:text-gray-400 text-center py-4">{{ __('messages.diseases.no_active_ingredients') }}</p>
                    @else
                        <ul class="space-y-2">
