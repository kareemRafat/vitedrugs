...
|---|---|---|---|---|
| 1.1 | - [x] Add `products.diseases` to the `load()` call in `ActiveIngredientController@show` | High | None |

## Milestone 2: Show — Remove unused eager load

| # | Task | Priority | Dependencies |
|---|---|---|---|---|
| 2.1 | - [x] Remove `products.dosageForm` from `load()` — it is never rendered on this page | Low | None |
...
| 3.2 | - [x] Run `./vendor/bin/pint` | Low | M1 |
