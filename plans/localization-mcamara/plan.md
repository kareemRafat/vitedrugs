...
|---|---|---|---|
| 0.1 | - [x] Run `composer require mcamara/laravel-localization` | High | None |
| 0.2 | - [x] Publish config: `php artisan vendor:publish --provider="Mcamara\LaravelLocalization\LaravelLocalizationServiceProvider"` | High | 0.1 |
| 0.3 | - [x] Edit `config/laravellocalization.php` — set `supportedLocales` to `en` + `ar`, `hideDefaultLocaleInURL` to `false`, `useAcceptLanguageHeader` to `false` | High | 0.2 |
| 0.4 | - [x] Set `APP_LOCALE=en` and `APP_FALLBACK_LOCALE=en` in `.env` | High | None |
| 0.5 | - [x] Create `lang/en/` and `lang/ar/` directories with `messages.php` files | High | None |
| 0.6 | - [x] Register middleware aliases in `bootstrap/app.php` (localize, localizationRedirect, localeSessionRedirect, localeViewPath) | High | 0.1 |
...
| # | Task | Priority | Dependencies |
|---|---|---|---|
| 1.1 | - [x] Wrap all public GET routes in `Route::group(['prefix' => LaravelLocalization::setLocale(), ...])` | High | 0.6 |
| 1.2 | - [x] Keep `admin/*`, `/sitemap.xml`, `/robots.txt` outside the locale group | High | 1.1 |
| 1.3 | - [x] Localize POST routes for login, register, logout inside the group | High | 1.1 |
| 1.4 | - [x] Update route references in controllers if needed | Medium | 1.1 |
...
| # | Task | Priority | Dependencies |
|---|---|---|---|
| 2.1 | - [x] Create `lang/en/messages.php` — all English UI strings (nav, auth forms, buttons, headings, placeholders, footer, empty states, search, pages) | High | 0.5 |
| 2.2 | - [x] Create `lang/ar/messages.php` — Arabic translations for all keys | High | 2.1 |
...
| # | Task | Priority | Dependencies |
|---|---|---|---|
| 3.1 | - [x] Translate `app/layouts/main-header.blade.php` — nav links, dropdown, sign-in button | High | 2.1 |
| 3.2 | - [x] Translate `app/layouts/main-sidebar.blade.php` — section labels, all nav items | High | 2.1 |
| 3.3 | - [x] Translate `app/layouts/footer.blade.php` — links, copyright line | Medium | 2.1 |
| 3.4 | - [x] Translate `app/layouts/guest.blade.php` — app name + footer text | Medium | 2.1 |
| 3.5 | - [x] Translate `app/auth/login.blade.php` — heading, email, password, remember, button, link | High | 2.1 |
| 3.6 | - [x] Translate `app/auth/register.blade.php` — heading, name, email, password, confirm, button, link | High | 2.1 |
| 3.7 | - [x] Translate `app/welcome.blade.php` — welcome text, buttons | Medium | 2.1 |
...
| # | Task | Priority | Dependencies |
|---|---|---|---|
| 5.1 | - [x] Create `resources/views/app/components/language-switcher.blade.php` — dropdown using `LaravelLocalization::getSupportedLocales()` and `getLocalizedURL()` with native names + flag | High | 0.3 |
| 5.2 | - [x] Include switcher in `app/layouts/main-header.blade.php` — next to user menu / nav links | High | 5.1 |
| 5.3 | - [x] Include switcher in `app/layouts/footer.blade.php` — alongside footer links | Medium | 5.1 |
| 5.4 | - [x] Add `dir="rtl"` class/attribute support for Arabic layout (optional but nice) | Low | 5.1 |
...
| 6.4 | - [x] Run `npm run build` | High | 3.1–3.20 |
| 6.5 | - [x] Run `./vendor/bin/pint` | High | 4.1–4.6 |
